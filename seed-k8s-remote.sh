...
for url in "${BASE_URL}/provider-manufacturing/cp" "${BASE_URL}/provider-qna/cp"
do
  newman run \
    --folder "Seed" \
    --env-var "HOST=$url" \
...
newman run \
  ...
  --env-var "HOST=${BASE_URL}/provider-catalog-server/cp" \
...
curl --location "${BASE_URL}/consumer/cs/api/identity/v1alpha/participants/" \
...
curl --location "${BASE_URL}/provider/cs/api/identity/v1alpha/participants/" \
--header 'Content-Type: application/json' \
--header "x-api-key: $API_KEY" \
--data "$DATA_PROVIDER"
...
curl -s --location "${BASE_URL}/issuer/cs/api/identity/v1alpha/participants/" \
--header 'Content-Type: application/json' \
--data "$DATA_ISSUER"
...
newman run \
  ...
  --env-var "ISSUER_ADMIN_URL=${BASE_URL}/issuer/ad" \
